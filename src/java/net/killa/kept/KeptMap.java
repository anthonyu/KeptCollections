...
public class KeptMap implements Map<String, String>, Synchronizable {
	 ...
	 */
	public KeptMap(ZooKeeper keeper, String znode, List<ACL> acl, CreateMode createMode) throws KeeperException, InterruptedException {
		...
		try {
			...
			KeptMap.LOG.debug("skipping creation of znode " + znode + " as it already exists");
		}
		...
		synchronized (this.map) {
			try {
				...
				for (String s : this.keeper.getChildren(this.znode, this.watcher))
					this.map.put(s, new String(this.keeper.getData(this.znode + '/' + s, this.watcher, null)));
	...
	private String putUnsynchronized(String key, String value) throws KeeperException, InterruptedException {
